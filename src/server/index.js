        ...
        if (dupCheck.length > 0)
            return res
                ...
                .json({
                    success: false,
                    message:
                        "Ya existe un modelo con ese nombre para este troquel",
                });
        ...
        if (dupCheck.length > 0)
            return res
                ...
                .json({
                    success: false,
                    message:
                        "Ya existe otro modelo con ese nombre para este troquel",
                });
...
app.delete("/api/troqueles/:id", async (req, res) => {
    try {
        ...
        await pool.query("DELETE FROM tbl_troqueles WHERE id_troquel = ?", [
            id,
        ]);
        res.json({
            success: true,
            ...
            if (!validRoles.includes(data.rol)) {
                return res
                    ...
                    .json({
                        success: false,
                        message:
                            "Rol no válido. Opciones: admin, supervisor, operator",
                    });
            }
            updates.push("rol = ?");
...
// Eliminar máquina (ID en query param)
        ...
        if (dupCheck.length > 0)
            return res
                ...
                .json({
                    success: false,
                    message:
                        "Ya existe un modelo con ese nombre para este molde",
                });
        ...
        if (dupCheck.length > 0)
            return res
                ...
                .json({
                    success: false,
                    message:
                        "Ya existe otro modelo con ese nombre para este molde",
                });
...
app.delete("/api/modelos-molde/:id", async (req, res) => {
    try {
        ...
        await pool.query("DELETE FROM tbl_modelos_molde WHERE id_modelo = ?", [
            id,
        ]);
...
`,
            ...
            if (new_status === "Reparando") {
                ...
                if (tipo_accion === "Falla de Molde")
                    motivo_entrada = "Falla de Molde";
                else if (tipo_accion === "Limpieza General")
                    motivo_entrada = "Limpieza General";
                else if (tipo_accion === "Cambio de Modelo")
                    motivo_entrada = "Cambio de Modelo";
