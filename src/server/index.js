...
query += ' WHERE m.troquel_id = ?';  
    ...
    res.json(rows);  
    	...
    	res.status(500).json({ success: false, message: 'Error al obtener modelos' });  
...
const [troquelCheck] = await pool.query('SELECT id_troquel FROM tbl_troqueles WHERE id_troquel = ?', [troquel_id]);  

    if (troquelCheck.length === 0) return res.status(400).json({ success: false, message: 'El troquel especificado no existe' });  
...
if (dupCheck.length > 0) return res.status(400).json({ success: false, message: 'Ya existe un modelo con ese nombre para este troquel' });  
...
const [result] = await pool.query(  
    ...
    res.status(201).json({ success: true, message: 'Modelo creado exitosamente', id_modelo: result.insertId });  
...
try {  
    ...
    const [check] = await pool.query('SELECT id_modelo FROM tbl_modelos_troquel WHERE id_modelo = ?', [id]);  

     

if (check.length === 0) return res.status(404).json({ success: false, message: 'Modelo no encontrado' });  

 

    	await pool.query('DELETE FROM tbl_modelos_troquel WHERE id_modelo = ?', [id]); 	 
    ...
    res.status(500).json({ success: false, message: 'Error al eliminar modelo' });  
...
const [estados] = await pool.query("SELECT id_estado as id, codigo, nombre, color, descripcion FROM tbl_estados WHERE activo = 1 ORDER BY orden ASC, nombre ASC"); 
...
const [asistencias] = await pool.query("SELECT id_asistencia_prensa as id, descripcion FROM tbl_asistencia_prensa WHERE activo = 1 ORDER BY descripcion ASC"); 
...
const [fallas] = await pool.query('SELECT id_fallas_catalogo, descripcion FROM tbl_fallas_catalogo WHERE activo = 1 ORDER BY descripcion'); 
...
const [troqueles] = await pool.query(`  
      ...
      LEFT JOIN tbl_ciclos_reparacion cr 
        ...
        ON cr.troquel_id = t.id_troquel AND cr.ciclo_activo = TRUE 
...
const [existing] = await pool.query('SELECT id_troquel FROM tbl_troqueles WHERE id_troquel = ?', 
...
const [existing] = await pool.query('SELECT * FROM tbl_troqueles WHERE id_troquel = ?', 
...
await pool.query('UPDATE tbl_troqueles SET estado = ? WHERE id_troquel = ?', 
...
const [existing] = await pool.query('SELECT id_troquel, nombre FROM tbl_troqueles WHERE id_troquel = ?', 
...
const [activos] = await pool.query("SELECT COUNT(*) as count FROM tbl_troqueles WHERE estado IN ('En prensa', 'Listo')"); 

const [reparando] = await pool.query("SELECT COUNT(*) as count FROM tbl_troqueles WHERE estado = 'Reparando'"); 

const [pendientes] = await pool.query("SELECT COUNT(*) as count FROM tbl_troqueles WHERE estado = 'Pendiente'"); 
...
const [repairs] = await pool.query(`  
    		...
    		FROM tbl_ciclos_reparacion cr  
    			...
    			JOIN tbl_troqueles t ON cr.troquel_id = t.id_troquel  
...
nivel_setup || null, grupo || null, prioridad_reparacion || 2 
...
const [results] = await pool.query('SELECT id_troquel, nombre, estado, año, modelo FROM tbl_troqueles WHERE id_troquel LIKE ? OR nombre LIKE ? OR modelo LIKE ?', 
