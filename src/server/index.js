	...
	try { 
		const [tables] = await pool.query("SHOW TABLES LIKE 'tbl_asistencia_prensa'"); 
			if (tables.length > 0) { 
				...
				//informacion por si falla al cargar 
				res.json([ 
	...
	try { 
		...
		troqueles.forEach(t => { 
			...
			groupedByYear[year].push({ 
				...
				//campos adici0onales para tener descripcion completea 
...
app.post('/api/troqueles', async (req, res) => { 
	try { 
		...
		if (!id_troquel || !nombre || !año) { 
			return res.status(400).json({success: false, message: 'ID, nombre y año son requeridos'}); 
		} 
...
app.patch('/api/troqueles/:id/status', async (req, res) => { 
	try { 
		...
		await pool.query('UPDATE tbl_troqueles SET estado = ? WHERE id_troquel = ?', [status, req.params.id]); 
...
app.delete('/api/troqueles/:id', async (req, res) => { 
	try { 
		const id = req.params.id; 
		const [existing] = await pool.query( 
			'SELECT id_troquel, nombre FROM tbl_troqueles WHERE id_troquel = ?',[id]); 
			if (existing.length === 0) { 
...
app.post('/api/actions/baja-troquel', async (req, res) => { 
	try { 
		...
		res.json({ 
			success: true, 
			message: 'Baja de troquel registrada exitosamente', 
			id: result.insertId 
		}); 
	} catch (error) { 
...
app.post('/api/actions/asistencia-prensa', async (req, res) => { 
	try { 
		...
		//obtener la descripcion para el tipo de accion
...
// Get history for a troquel (updated to handle both types) 
...
LEFT JOIN tbl_asistencia_prensa ap ON h.id_historial = ap.id_asistencia_prensa 
...
console.log(`Baja Troquel API: http://localhost:${PORT}/api/actions/baja-troquel`); 

console.log(`Asistencia Prensa API: http://localhost:${PORT}/api/actions/asistencia-prensa`); 
